...
#############################################################
drawing_BP <- function(in_pattern, in_colour){
  BP_sig_annotation_file <- paste(c('/Users/uqjjian3/Statin_project/Statins_HA1E_CMAP_July27/25_Annotating_GO_BP_levels/statin_vs_antidepressant_gProfiler2_HA1E/Individual_statin_vs_antidepressant_gProfiler2_', in_pattern, '_annotated_level2_ancestor.txt'), collapse = '')
  statin_antidep_BP_df <- fread(BP_sig_annotation_file) 
  ...
  curr_title <- paste(c('GO Biological processes enriched in genes perturbed in ', in_pattern), collapse = '')
  curr_fig <- ggplot(data=statin_antidep_annotation_summary_sort, aes(x=factor(label, levels = statin_antidep_annotation_summary_sort$label), y=Perc)) +
    geom_bar(stat="identity", fill=in_colour, alpha = 0.5)+
    ylab('Percentage of significant biological pathway (%)')+
    geom_text(aes(label = Frequency), vjust = 0) +  
    ggtitle(label = curr_title)+
    ylim(0, 100)+
  ...
  out_fig_file <- paste(c('/Users/uqjjian3/Statin_project/Statins_HA1E_CMAP_July27/25_Annotating_GO_BP_levels/statin_vs_antidepressant_gProfiler2_HA1E/Individual_statin_vs_antidepressant_gProfiler2_', in_pattern, '_annotated_level2_ancestor_count.pdf'), collapse = '')
  ggsave(
    out_fig_file,
    plot = curr_fig,
    device = NULL,
