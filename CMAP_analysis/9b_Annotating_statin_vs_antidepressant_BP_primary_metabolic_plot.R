...
#############################################################
### Getting the level 2 terms of biological process (GO:0044238)
#############################################################
BP_root_term <- c('GO:0044238')
primary_metabolic_terms_df <- as.data.frame(GOBPCHILDREN[c('GO:0044238')])
primary_metabolic_terms_df <- primary_metabolic_terms_df[which(primary_metabolic_terms_df$RelationshipType == 'isa'),]
primary_metabolic_terms <- primary_metabolic_terms_df$go_id
...
Creating_annotation_file <- function(in_pattern){
  ...
  statin_antidep_BP_df$ancestor_primary_metabolic <- c('NA')
  for (BP_ID in statin_antidep_BP_df$GO_BP_ID){
    ...
    for (curr_ancestor in c(curr_term_ancestor_vector, BP_ID)){
      if (curr_ancestor %in% primary_metabolic_terms){
        annotated_ancestor <- c(annotated_ancestor, curr_ancestor)
      }
    }
    annotated_ancestor_string <- paste(annotated_ancestor, collapse = '|')
    statin_antidep_BP_df$ancestor_primary_metabolic <- ifelse(statin_antidep_BP_df$GO_BP_ID == BP_ID, annotated_ancestor_string, statin_antidep_BP_df$ancestor_primary_metabolic)
    
    Curr_term <- Term(as.list(GOTERM[c(BP_ID)])[[1]])
    statin_antidep_BP_df[which(statin_antidep_BP_df$GO_BP_ID == BP_ID), 'Term'] <- Curr_term
  }
  
  BP_sig_annotation_file <- paste(c('/Users/uqjjian3/Statin_project/Statins_HA1E_CMAP_July27/25_Annotating_GO_BP_levels/statin_vs_antidepressant_gProfiler2_HA1E/Individual_statin_vs_antidepressant_gProfiler2_', in_pattern, '_annotated_primary_metabolic_ancestor.txt'), collapse = '')
...
counting_frequency <- function(in_pattern){
  ...
  statin_antidep_annotation_vector_unsplit <- statin_antidep_BP_df$ancestor_primary_metabolic
  ...
  statin_antidep_annotation_count_file <- paste(c('/Users/uqjjian3/Statin_project/Statins_HA1E_CMAP_July27/25_Annotating_GO_BP_levels/statin_vs_antidepressant_gProfiler2_HA1E/Individual_statin_vs_antidepressant_gProfiler2_', in_pattern, '_annotated_primary_metabolic_ancestor_count.txt'), collapse = '')
...
out_fig_file <- '/Users/uqjjian3/Statin_project/Statins_HA1E_CMAP_July27/25_Annotating_GO_BP_levels/statin_vs_antidepressant_gProfiler2_HA1E/Individual_statin_vs_antidepressant_gProfiler2_primary_metabolic_ancestor_count.pdf'
ggsave(
  out_fig_file,
  plot = curr_fig,
  device = NULL,
